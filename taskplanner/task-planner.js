...
let loginInfo = `
    <form action="/store" method="get">
        <h1><center>Task Planner</center></h1><br/>
        <h3>Add Task</h3>
    ...
    if(req.url=="/"){
        ...
        if(pathInfo=="/store"){  
            ...
            fs.readFile('tasklog.json',function(err,data){
                ...
                fs.writeFile("tasklog.json",empString,(err)=> {
                if(!err){
             ...
             res.setHeader("content-type","text/html"); 
              var data=fs.readFileSync('tasklog.json')
        ...
        res.end(loginInfo);
        var obj=new Array()
           var jsondata= fs.readFileSync('tasklog.json')
